...
Note: For ease of use, the command-line tool is named `rp`. You will need to use `rp` to execute commands.
...
just build    # Build to ./release/rp
```
...
```bash
rp -m "Here is a video link: https://youtu.be/xgXFCOoQJVc"
```
...
```bash
rp iu_folder/ -m "Hi! Here is today's IU report" -c iu_wiki.md -p 1993
```
